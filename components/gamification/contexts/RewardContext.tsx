...
interface RewardsContextType {
  ...
  resetAnimationState: (type: 'levelUp' | 'streakMilestone' | 'badgeUnlocked' | 'challengeCompleted') => void;
...
export const RewardsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const resetAnimationState = (type: 'levelUp' | 'streakMilestone' | 'badgeUnlocked' | 'challengeCompleted') => {
  ...
  const getNextMilestone = () => {
    return STREAK_MILESTONES.find(m => m > userRewards.streakCount) || STREAK_MILESTONES[STREAK_MILESTONES.length - 1];
  };

  const getProgressToNextMilestone = () => {
    const nextMilestone = getNextMilestone();
    const prevMilestone = STREAK_MILESTONES.filter(m => m < userRewards.streakCount).pop() || 0;
